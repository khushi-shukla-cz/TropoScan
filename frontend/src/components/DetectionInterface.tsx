...
const DetectionInterface = () => {
  ...
  useEffect(() => {
    ...
    const loadSampleImages = async () => {
      try {
        const response = await fetch(API_ENDPOINTS.sampleImages);
        if (response.ok) {
  ...
  const loadSamplePreview = async (sampleId: string) => {
    try {
      const response = await fetch(API_ENDPOINTS.samplePreview(sampleId));
      if (response.ok) {
        const data = await response.json();
        if (data.success) {
  ...
  const processSampleImage = async (sampleId: string) => {
    ...
    try {
      const response = await fetch(API_ENDPOINTS.sample(sampleId), {
        method: 'POST'
      });
