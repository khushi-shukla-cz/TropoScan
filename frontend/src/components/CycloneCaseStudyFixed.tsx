...
const CycloneCaseStudy: React.FC = () => {
  ...
  const fetchCaseStudies = async () => {
    try {
      const response = await fetch(API_ENDPOINTS.caseStudies);
  ...
  const processCaseStudy = async (caseId: string) => {
    ...
    try {
      const response = await fetch(API_ENDPOINTS.caseStudy(caseId), {
        method: 'POST'
      });
      const data = await response.json();
      setResult(data);
  ...
  const processUploadedImage = async () => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.uploadCaseStudy, {
        method: 'POST',
        body: formData
      });
